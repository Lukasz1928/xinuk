...
final case class WolfCell(smell: SmellArray, energy: Energy, lifespan: Long, signalInd: Int) extends SmellingCell {
...
object WolfAccessible {
  ...
  def unapply(arg: RabbitCell)(implicit config: PredpreyConfig): WolfAccessible[WolfCell] =
    new WolfAccessible[WolfCell] {
      override def withWolf(energy: Energy, lifespan: Long): WolfCell = WolfCell(arg.smellWith(SignalArray(config.wolfInitialSignal)), energy + config.rabbitEnergeticCapacity, lifespan, config.wolfSignalIndex)
    }

  def unapply(arg: EmptyCell)(implicit config: PredpreyConfig): WolfAccessible[WolfCell] =
    new WolfAccessible[WolfCell] {
      override def withWolf(energy: Energy, lifespan: Long): WolfCell = WolfCell(arg.smellWith(SignalArray(config.wolfInitialSignal)), energy, lifespan, config.wolfSignalIndex)
    }

  def unapply(arg: BufferCell)(implicit config: PredpreyConfig): WolfAccessible[BufferCell] =
    new WolfAccessible[BufferCell] {
      override def withWolf(energy: Energy, lifespan: Long): BufferCell = BufferCell(WolfCell(arg.smellWith(SignalArray(config.wolfInitialSignal)), energy, lifespan, config.wolfSignalIndex))
    }
