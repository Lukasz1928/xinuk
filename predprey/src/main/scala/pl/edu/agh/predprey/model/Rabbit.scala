...
final case class RabbitCell(smell: SmellArray, energy: Energy, lifespan: Long) extends SmellingCell {
...
object RabbitAccessible {
  ...
  def unapply(arg: LettuceCell)(implicit config: PredpreyConfig): RabbitAccessible[RabbitCell] =
    new RabbitAccessible[RabbitCell] {
      override def withRabbit(energy: Energy, lifespan: Long): RabbitCell = RabbitCell(arg.smellWith(SignalArray(config.rabbitInitialSignal)), energy + config.lettuceEnergeticCapacity, lifespan)
    }

  def unapply(arg: EmptyCell)(implicit config: PredpreyConfig): RabbitAccessible[RabbitCell] =
    new RabbitAccessible[RabbitCell] {
      override def withRabbit(energy: Energy, lifespan: Long): RabbitCell = RabbitCell(arg.smellWith(SignalArray(config.rabbitInitialSignal)), energy, lifespan)
    }

  def unapply(arg: BufferCell)(implicit config: PredpreyConfig): RabbitAccessible[BufferCell] =
    new RabbitAccessible[BufferCell] {
      override def withRabbit(energy: Energy, lifespan: Long): BufferCell = BufferCell(RabbitCell(arg.smellWith(SignalArray(config.rabbitInitialSignal)), energy, lifespan))
    }
